...
class CsvQASrc(QASrc):
    ...
    def load_data(self):
        super().load_data()
        data = []
        with open(self.file, 'r', encoding='utf8') as ifile:
            for line in ifile:
                fields = line.strip().split('\t')
                question = fields[self.question_col]
                answers = eval(fields[self.answers_col])
                id = None
                if self.id_col >= 0:
...
class CsvCtxSrc(RetrieverData):
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage], start_idx: int, end_idx: int):
        super().load_data()
        with open(self.file, 'r', encoding='utf8') as ifile:
            reader = csv.reader(ifile, delimiter="\t")
            passage_idx = 0
            for row in reader:
                ...
                # Only read this shard
                if passage_idx < start_idx:
                    passage_idx += 1
                    continue
                if passage_idx >= end_idx:
                    break
