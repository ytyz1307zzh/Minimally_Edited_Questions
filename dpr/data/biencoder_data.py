...
from ..utils.data_utils import read_data_from_json_files, read_data_from_jsonl_files, Tensorizer
...
class JsonQADataset(Dataset):
    ...
    def load_data(self, local_rank):
        ...
        if path.endswith(".json"):
            data = read_data_from_json_files(self.data_files, local_rank)
        elif path.endswith(".jsonl"):
            data = read_data_from_jsonl_files(self.data_files, local_rank)
        else:
            raise ValueError(f"Invalid data format {path}")
        # filter those without positive ctx
        self.data = [r for r in data if len(r["positive_ctxs"]) > 0]
        logger.info("Total cleaned data size: {}".format(len(self.data)))

    def read_wiki(self, wiki_path):
        pid2text = {}
        ...
        with open(wiki_path, 'r', encoding='utf8') as fin:
            ...
            for line in fin:
                ...
                pid2text[idx] = {"text": text, "title": title}
        print(f'Read {len(pid2text)} passages from Wikipedia, time elapse: {time.time() - start_time}')
        return pid2text

    def load_data_memory_friendly(self, local_rank):
        # wiki_pid2text = self.read_wiki(wiki_path)
        self.data_files = get_dpr_files(self.file)
        self.data = read_data_from_jsonl_files(self.data_files, local_rank)
        # self.wiki_pid2text = wiki_pid2text

    def __getitem__(self, index):
        raise NotImplementedError("Please call get_item instead!")

    def get_item(self, data_idx, pos_question_idx=None, hard_neg_question_idx=None) -> BiEncoderSample:
        json_sample = self.data[data_idx]
        r = BiEncoderSample()
        ...
        # hard negative passages are only presented as ids
        # hard_negative_ctx_ids = json_sample["hard_negative_ctxs"] if "hard_negative_ctxs" in json_sample else []
        # Sample a hard negative passage (do it here to save memory)
        # sampled_hard_negative_ctx_id = random.sample(hard_negative_ctx_ids, 1)
        # So hard_negative_ctxs will be length 1; Further sampling will definitely select this only one
        # hard_negative_ctxs = [self.wiki_pid2text[id_] for id_ in sampled_hard_negative_ctx_id]
        # Leave sampling to later step because in validation we need more than one hard negatives
        # hard_negative_ctxs = [self.wiki_pid2text[id_] for id_ in hard_negative_ctx_ids]
        ...
        if pos_question_idx is not None and hard_neg_question_idx is not None:
            r.positive_question = json_sample["positive_questions"][pos_question_idx]
            r.hard_negative_question = json_sample["hard_negative_questions"][hard_neg_question_idx]
        else:
            r.positive_question = None
            r.hard_negative_question = None
